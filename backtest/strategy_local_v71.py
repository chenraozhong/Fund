...
class LocalV71Strategy(Strategy):
    """v7.1: v7.0回撤精调 — 趋势门槛收紧+sigmoid微调+回撤护栏+trailing drawdown"""
    def __init__(self, params=None):
        default = {
            ...
            # v7.1 精调参数
            'atr_limit_default': 2.5, 'atr_limit_trending': 3.5,  # v7.0: 4.0→3.5
            'streak_accel_5': 1.15, 'streak_accel_3': 1.05,       # v7.0: 1.3/1.1→1.15/1.05
            'streak_decay_5': 0.45, 'streak_decay_3': 0.80,       # v7.0: 0.55/0.87→0.45/0.80
            'bb_sigmoid_center': 90,                                # v7.0: 85→90
            'trend_mode_threshold': 20,                             # v7.0: 10→20
            'dd_guardrail_center': 6,                               # 回撤护栏中心
        }
        if params: default.update(params)
        super().__init__('v7.1决策模型', default)
    ...
    def generate_signal(self, df, i, market_df=None):
        ...
        # [v7.1] 趋势模式: 门槛收紧 + 回撤<5%
        is_trend_mode = (regime == 'trending') and (hist > 0) and (raw > 0) and (cur_dd < 5)
        # 需要模拟compositeScore>20, 用raw信号强度代替
        is_trend_mode = is_trend_mode and (raw > 0.3)

        # [v7.1 sigmoid] BB高位: center=90
        if raw > 0 and not is_trend_mode:
            bb_reduction = 1 - _sigmoid(pctb, p['bb_sigmoid_center'], 0.12)
            raw *= bb_reduction
        ...
        # [v7.1] 回撤护栏: 回撤>3%时sigmoid缩减买入信号
        if raw > 0 and cur_dd > 3:
            dd_reduction = 1 - _sigmoid(cur_dd, p['dd_guardrail_center'], 0.4)
            raw *= dd_reduction

        # [v7.1] 亏损观望: 弱正信号+趋势负 → 归零
