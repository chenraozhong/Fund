...
// 模型版本号
// ============================================================
const FORECAST_MODEL_VERSION = 'v7.0';  // 预测模型版本（v7: 连涨加速+趋势模式ATR4x）
const DECISION_MODEL_VERSION = 'v7.0';  // 决策模型版本（v7: 五派评审修复 — 亏损观望区+组合风控+sigmoid软化+趋势模式+卖出精简+分级熔断）
...
async function computeDecision(fundId: number | string, realtimeNav: number): Promise<any> {
  ...
  if (nav > costNav && costNav > 0) {
    ...
    if (cyclePhase === 'euphoria' && swingShares > 0) {
      ...
      // [v7 趋势模式] 趋势未反转 → 持有不卖, 趋势反转(compositeScore转空+MACD死叉) → 卖
      if (compositeScore <= -10 && technical.macd.histogram < 0) {
        ...
        // 趋势中回调 → 加仓机会
        action = 'buy';
        opAmount = Math.round(rawBase * 1.2 * buyConviction * bf);
  ...
  // [v7] 趋势模式判定: trending+多头排列 → 放宽拦截
  const isTrendMode = isTrending && technical.macd.histogram > 0 && compositeScore > 10;
  ...
  // [v7 sigmoid] 布林带高位→连续衰减(替代v6.2硬阈值85)
  // 趋势模式豁免: strong_up+MACD递增时%B>85是正常的
...
function computeForecastCore(
  ...
  // [v7] 连涨/连跌处理: 趋势模式加速, 非趋势衰减
  if (forecastRegime === 'trending') {
    // 趋势确认 → 连涨加速而非衰减
    if (Math.abs(streak) >= 5) trendFactor *= 1.3;       // v6.2: 0.35 → v7: 1.3
    else if (Math.abs(streak) >= 3) trendFactor *= 1.1;   // v6.2: 0.87 → v7: 1.1
  } else {
    // 震荡/高波动 → 保留衰减但更温和
    if (Math.abs(streak) >= 5) trendFactor *= 0.55;       // v6.2: 0.35 → v7: 0.55
  ...
  // [v7] ATR限幅: 趋势模式4x, 极端地缘4x, 默认2.5x
  const atrLimitMult = (forecastRegime === 'trending') ? 4.0
