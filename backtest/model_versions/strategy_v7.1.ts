...
// 模型版本号
// ============================================================
const FORECAST_MODEL_VERSION = 'v7.1';  // 预测模型版本（v7.1: 连涨加速温和化+回撤护栏）
const DECISION_MODEL_VERSION = 'v7.1';  // 决策模型版本（v7.1: 回撤精调 — 趋势门槛收紧+sigmoid微调+回撤护栏+连涨温和化）
...
async function computeDecision(fundId: number | string, realtimeNav: number): Promise<any> {
  ...
  // === E4. 分级熔断 [v7: 15%审查→20%减仓→25%强制卖出] ===
  ...
  if (totalLossPct > 25) cbLevel = 'critical';       // 25%+: 强制卖出活仓50%
  else if (totalLossPct > 20) cbLevel = 'reduce';     // 20-25%: 暂停买入+缩减仓位
  else if (totalLossPct > 15) cbLevel = 'review';     // 15-20%: 暂停买入,仅允许止损

  // === E5. 组合级风控 [v7新增: 单日总买入不超组合10%] ===
  const portfolioValue = getPortfolioValue();
  const todayBuyTotal = getTodayBuyTotal();
  const dailyBuyLimit = Math.max(portfolioValue * 0.10, 5000); // 至少5000
  const dailyBuyRemaining = Math.max(0, dailyBuyLimit - todayBuyTotal);

  // === F. 动态止盈线 [v7: trailing stop模式, 趋势中更宽] ===
  const isTrending = technical.trend === 'strong_up' || technical.trend === 'up';
  const trailingMult = isTrending ? 5 : 3; // 趋势中ATR×5, 否则ATR×3
  ...
  if (nav > costNav && costNav > 0) {
    ...
    // [索罗斯修正] Capitulation分步建仓：先30%试探
      ...
      if (fundOk) {
        ...
        reasoning.push(`[v7观望] 综合评分${compositeScore}偏空+基本面弱（${fundScore.score}分），暂不加仓`);
      }
    } else {
      // [v7修正] compositeScore<-15 → 观望，不再逆向买入
      action = 'hold'; confidence = 40; urgency = 'low';
      reasoning.push(`[v7观望] 综合评分${compositeScore}<-15，信号偏空，等待明确见底信号`);
    }
  }
  ...
  // [v7.1] 趋势模式门槛收紧(v7.0: >10太宽松导致回撤增加)
  // 要求: 趋势确认 + MACD多头 + 综合评分>20 + 短期无大回撤
  const recentDrawdown = riskMetrics.currentDrawdown;
  const isTrendMode = isTrending && technical.macd.histogram > 0 && compositeScore > 20 && recentDrawdown < 5;

  // [v7 sigmoid] 预测跌→连续衰减买入(替代v6.2硬阈值1%)
  if (action === 'buy' && forecast.direction === 'down' && opAmount > 0 && !isTrendMode) {
  ...
  // [v7.1 sigmoid] 布林带高位→连续衰减(v7.1: center 85→90更保守)
  if (action === 'buy' && opAmount > 0 && !isTrendMode) {
    const pctB = technical.bollingerBands.percentB;
    // sigmoid: %B=75→衰减到95%, %B=90→衰减到50%, %B=105→衰减到10%
    const bbReduction = 1 - sigmoid(pctB, 90, 0.12);
    if (bbReduction < 0.15) {
      action = 'hold'; opShares = 0; opAmount = 0;
  ...
  // [v7.1] 回撤护栏: 当前基金回撤>5%时缩减买入
  if (action === 'buy' && opAmount > 0 && riskMetrics.currentDrawdown > 3) {
    // 回撤3%→衰减到85%, 5%→衰减到60%, 10%→衰减到20%
    const ddReduction = 1 - sigmoid(riskMetrics.currentDrawdown, 6, 0.4);
    opShares = r4(opShares * ddReduction);
    opAmount = Math.round(opShares * nav);
    if (ddReduction < 0.9) {
      reasoning.push(`[v7.1回撤护栏] 当前回撤${riskMetrics.currentDrawdown.toFixed(1)}%→买入缩减至${Math.round(ddReduction*100)}%`);
    }
  }

  // [v7] 地缘恐慌→sigmoid衰减(保留，非趋势模式)
  if (action === 'buy' && geoRisk && geoRisk.riskScore <= -20 && opAmount > 0) {
    // sigmoid: riskScore=-20→衰减到80%, -50→衰减到25%, -80→衰减到5%
  ...
  // [v7新增] 组合级日买入限额
  if (action === 'buy' && opAmount > 0 && opAmount > dailyBuyRemaining) {
    if (dailyBuyRemaining <= 0) {
      action = 'hold'; opShares = 0; opAmount = 0;
      reasoning.push(`[v7组合风控] 今日买入已达限额¥${Math.round(dailyBuyLimit)}（组合10%），暂停`);
    } else {
...
function computeForecastCore(
  ...
  // [v7.1] 连涨/连跌处理: 趋势模式温和加速(v7.0回撤分析: 1.3太激进)
  if (forecastRegime === 'trending') {
    if (Math.abs(streak) >= 5) trendFactor *= 1.15;       // v7.0: 1.3 → v7.1: 1.15
    else if (Math.abs(streak) >= 3) trendFactor *= 1.05;   // v7.0: 1.1 → v7.1: 1.05
  } else {
    if (Math.abs(streak) >= 5) trendFactor *= 0.45;       // v7.0: 0.55 → v7.1: 0.45
    else if (Math.abs(streak) >= 3) trendFactor *= 0.80;   // v7.0: 0.87 → v7.1: 0.80
  }
