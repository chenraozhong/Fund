...
// 模型版本号
// ============================================================
const FORECAST_MODEL_VERSION = 'v7.3';  // 预测模型版本（v7.3: 混合架构 sigmoid+硬底线, 非趋势恢复强衰减）
const DECISION_MODEL_VERSION = 'v7.3';  // 决策模型版本（v7.3: 卡尔玛3.051冠军 — v7.2+硬底线%B>95/ts<-35+非趋势streak×0.35）
...
// 模型版本配置
// ============================================================
type ModelVersionId = 'v6.2' | 'v7.2' | 'v7.3';
...
const DEFAULT_MODEL: ModelVersionId = 'v7.3';
...
async function computeDecision(fundId: number | string, realtimeNav: number, modelVersion?: ModelVersionId): Promise<any> {
  ...
  if (modelCfg.circuitBreakerMode === 'tiered') {
    ...
    // v6.2: 仅25%单级
    if (totalLossPct > 25) cbLevel = 'review'; // 仅暂停买入,不强制卖出
  }
