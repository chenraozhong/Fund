...
// 模型版本号
// ============================================================
const FORECAST_MODEL_VERSION = 'v7.2';  // 预测模型版本（v7.2: v7.0基础+连涨1.2x+ATR3.5x+趋势门槛15）
const DECISION_MODEL_VERSION = 'v7.2';  // 决策模型版本（v7.2: 回滚v7.1过度修复, 在v7.0上精调 — 连涨×1.2+趋势>15+ATR3.5+trailing drawdown）
...
async function computeDecision(fundId: number | string, realtimeNav: number): Promise<any> {
  ...
  // [v7.2] 趋势模式: compositeScore>15(v7.0:>10, v7.1:>20矫枉过正)
  const isTrendMode = isTrending && technical.macd.histogram > 0 && compositeScore > 15;
  ...
  // [v7.2 sigmoid] 布林带高位→连续衰减(center=85, v7.1的90太保守)
  ...
  // [v7.2] 回撤护栏已删除(v7.1证实过度杀伤：收益从33%暴跌到16%)
  // 回撤控制由分级熔断+trailing drawdown卖出机制负责
