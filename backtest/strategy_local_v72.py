...
class LocalV72Strategy(Strategy):
    """v7.2: v7.0基础+连涨×1.2+趋势>15+ATR3.5+trailing drawdown+无回撤护栏"""
    def __init__(self, params=None):
        default = {
            ...
            'streak_accel_5': 1.20, 'streak_accel_3': 1.08,
            'streak_decay_5': 0.55, 'streak_decay_3': 0.87,
            'bb_sigmoid_center': 85,
            'trend_mode_threshold': 15,
        }
        if params: default.update(params)
        super().__init__('v7.2决策模型', default)
    ...
    def generate_signal(self, df, i, market_df=None):
        ...
        # [v7.2] sigmoid BB: center=85, 趋势豁免
        if raw > 0 and not is_trend_mode:
            raw *= 1 - _sigmoid(pctb, p['bb_sigmoid_center'], 0.12)

        # sigmoid趋势崩坏
        if raw > 0 and ts < 0:
            raw *= 1 - _sigmoid(abs(ts), 25, 0.12)

        # [v7.2] 无回撤护栏(v7.1证实过度杀伤)
