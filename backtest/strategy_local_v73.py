#!/usr/bin/env python3
"""v7.3 = v7.2 + 混合架构(sigmoid+硬底线), 非趋势streak恢复v6.2"""
...
class LocalV73Strategy(Strategy):
    """v7.3: v7.2 + 混合架构(sigmoid+硬底线) + 非趋势恢复强衰减"""
    def __init__(self, params=None):
        default = {
            ...
            # v7.3 = v7.2 + 混合架构
            'streak_trending_5': 1.20, 'streak_trending_3': 1.08,   # 趋势: 保持v7.2加速
            'streak_other_5': 0.35, 'streak_other_3': 0.80,         # 非趋势: 恢复v6.2强衰减!
            'bb_hard_ceiling': 95,    # 硬底线: %B>95归零
            'ts_hard_floor': -35,     # 硬底线: ts<-35 ×0.25
        }
        if params: default.update(params)
        super().__init__('v7.3决策模型', default)
    ...
    def generate_signal(self, df, i, market_df=None):
        ...
        # [v7.3] 连涨: 趋势保持v7.2加速, 非趋势恢复v6.2强衰减
        if regime == 'trending':
