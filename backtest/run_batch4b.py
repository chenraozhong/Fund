#!/usr/bin/env python3
import os,sys,json,math,time as _time,traceback
...
for idx, (code, name, sector) in enumerate(FUNDS):
    _time.sleep(0.5)
    try:
        print(f'[{idx+1}/{len(FUNDS)}] Fetching {name}({code})...', flush=True)
        df = fetch_fund_nav_history(code, 1825)
        if df is None or len(df) < 60:
            print(f'  SKIP - insufficient data ({0 if df is None else len(df)} days)', flush=True)
            continue
